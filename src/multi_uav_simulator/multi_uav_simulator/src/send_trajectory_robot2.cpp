...
public:
    WaypointController()
    {
        ...
        if (nh_.getParam("robot_2/optimizer_type", optimizer_type_))
        {
        ...
        current_state_sub_ = nh_.subscribe("/robot_2/current_state", 10, &WaypointController::currentStateCallback, this);
        desired_state_pub2_ = nh_.advertise<geometry_msgs::Point>("/robot_2/desired_state", 10);
        waypoint_index_ = 0;
        publish_allowed_ = false;
        std::vector<double> position;
        if (nh_.getParam("robot_2/position", position))
        {
            if (position.size() == 3)
            {
...
private:
    ...
    void publishNextWaypoint()
    {
        ...
        if (waypoint_index_ < path_.size())
        {
            ...
            desired_state_pub2_.publish(point_msg);
        }
        else
        {
    ...
    ros::Publisher desired_state_pub2_;
    std::vector<geometry_msgs::PoseStamped> path_;
    geometry_msgs::Point current_location_;
    int waypoint_index_;
    bool publish_allowed_;
    const double threshold_ = 0.05;  // threshold to consider if the drone is close to the waypoint
};

int main(int argc, char** argv)
{
    ros::init(argc, argv, "waypoint_controller_robot2");
