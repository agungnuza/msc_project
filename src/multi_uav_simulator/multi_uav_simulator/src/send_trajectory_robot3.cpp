...
public:
    WaypointController()
    {
        ...
        if (nh_.getParam("robot_3/optimizer_type", optimizer_type_))
        {
        ...
        current_state_sub_ = nh_.subscribe("/robot_3/current_state", 10, &WaypointController::currentStateCallback, this);
        desired_state_pub3_ = nh_.advertise<geometry_msgs::Point>("/robot_3/desired_state", 10);
        waypoint_index_ = 0;
        publish_allowed_ = false;
        std::vector<double> position;
        if (nh_.getParam("robot_3/position", position))
        {
            if (position.size() == 3)
            {
...
{
    ros::init(argc, argv, "waypoint_controller_robot3");
