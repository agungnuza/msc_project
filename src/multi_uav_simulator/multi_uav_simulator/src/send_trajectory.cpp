...
public:
    WaypointController()
    {
        ...
        if (nh_.getParam("robot_1/optimizer_type", optimizer_type_))
        {
        ...
        current_state_sub_ = nh_.subscribe("/robot_1/current_state", 10, &WaypointController::currentStateCallback, this);
        desired_state_pub_ = nh_.advertise<geometry_msgs::Point>("/robot_1/desired_state", 10);
        waypoint_index_ = 0;
        publish_allowed_ = false;
        std::vector<double> position;
        if (nh_.getParam("robot_1/position", position))
        {
            if (position.size() == 3)
            {
...
private:
    ...
    void publishNextWaypoint()
    {
        ...
        if (waypoint_index_ < path_.size())
        {
            ...
            desired_state_pub_.publish(point_msg);
        }
        else
        {
