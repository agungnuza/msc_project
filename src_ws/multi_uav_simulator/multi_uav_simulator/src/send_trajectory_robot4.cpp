...
public:
    WaypointController()
    {
        ...
        if (nh_.getParam("robot_4/optimizer_type", optimizer_type_))
        {
        ...
        current_state_sub_ = nh_.subscribe("/robot_4/current_state", 10, &WaypointController::currentStateCallback, this);
        desired_state_pub4_ = nh_.advertise<geometry_msgs::Point>("/robot_4/desired_state", 10);
        waypoint_index_ = 0;
        publish_allowed_ = false;
        std::vector<double> position;
        if (nh_.getParam("robot_4/position", position))
        {
            if (position.size() == 3)
            {
...
private:
    ...
    void publishNextWaypoint()
    {
        ...
        if (waypoint_index_ < path_.size())
        {
            ...
            desired_state_pub4_.publish(point_msg);
        }
        else
        {
    ...
    ros::Publisher desired_state_pub4_;
...
{
    ros::init(argc, argv, "waypoint_controller_robot4");
