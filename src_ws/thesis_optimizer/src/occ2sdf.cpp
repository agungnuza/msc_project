...
void PCCallback(const sensor_msgs::PointCloud2::ConstPtr& msg)
{
    ...
    if (!sdf)
    {
        ...
        //calculate abs distance between current position and end position
...
{
    //get current position from robot_1/current_state
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber current_state_sub_ = nh.subscribe("/robot_1/current_state", 10, CurrStateCallback);


    
    pub = nh.advertise<sensor_msgs::PointCloud2>("/sdf_cloud", 1);
    path_pub_init = nh.advertise<nav_msgs::Path>("/trajectory_init", 1);
    path_pub_opt = nh.advertise<nav_msgs::Path>("/trajectory_opt_occ2sdf", 1);
